...
const StyledCard = styled(Card)(({ theme }) => ({
  ...
  backgroundColor: '#ffeb3b', // Kid-friendly color
}));
...
const KidExerciseDashboard = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const [kid, setKid] = useState(location.state?.kid || JSON.parse(localStorage.getItem('kid'))); // Retrieve kid data from state or local storage
  ...
  useEffect(() => {
    if (kid && kid._id) {
      // Fetch kid's exercises data from the server
      axios.get(`http://localhost:5000/api/kids/${kid._id}/exercises`)
        .then(response => {
  ...
  useEffect(() => {
    ...
    socket.on('login_event', (data) => {
      ...
      if (data.kid) {
        console.log('Navigating to kid exercises:', data.kid._id);
        navigate(`/kid/exercises/${data.kid._id}`);
      }
    });
  ...
  const handlePerformExercise = () => {
    ...
    axios.post('http://localhost:5000/api/start_exercise')
      .then(response => {
        console.log('Exercise response:', response.data);  // Add this line for debugging
        setSnackbarMessage(`Exercise completed successfully. Output: ${response.data.output}`);
        setSnackbarSeverity('success');
        setSnackbarOpen(true);
        setExerciseResult(`Exercise completed successfully. Output: ${response.data.output}`); // Set the exercise result
      })
      .catch(error => {
        console.error('Error performing exercise:', error);  // Add this line for debugging
    ...
    return (
      <Container maxWidth="md">
        <Typography variant="h4" component="h1" gutterBottom>
          No kid data available. Please log in again.
        </Typography>
      </Container>
    );
  }
      ...
      <Container maxWidth="md" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <Box my={4} flexGrow={1}>
          <Typography variant="h4" gutterBottom>
            {kid.name}'s Exercise Plan
            <img src="/Say Hello PNG.jpeg" alt="Say Hello" style={{ width: 30, height: 30, marginLeft: 10 }} />
          </Typography>
          ...
          <Typography variant="h6" gutterBottom>
            ...
            <img src="/squatKid.png" alt="Squat Kid" style={{ width: 30, height: 30, marginLeft: 10 }} />
          </Typography>
          {exercises.length > 0 && (
...
export default KidExerciseDashboard;
